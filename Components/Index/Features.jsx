      ...
      <div className="w-full mx-auto lg:flex justify-center mx-auto   h-auto  overflow-hidden  ">
        {
          <div className="w-full gap-6 md:grid-cols-2 md:grid   mx-auto md:pl-24 sm:p-48 lg:pl-24 pr-6 md:p-12 sm:p-2 ">
            {items.map((item, index) => (
              ...
              >
                <div
                  className="bg-blue-200 w-10 h-10 rounded-full text-blue-500 p-2"
                  dangerouslySetInnerHTML={{ __html: item.svg }}
                />

                <h2 className="font-nunito text-blue-800 font-bold">
                  {item.title}
                </h2>
                <p className="font-rubik text-gray-700 ">{item.description}</p>
                <img
                  src="../images/features-card-shape-1.png"
                  class="absolute top-0 left-14 w-24 h-12 opacity-0 hover:opacity-100 transition-opacity duration-300 transition-transform hover:scale-110"
                />
                <img
                  src="../images/features-card-shape-1.png"
                  class="absolute bottom-0 right-14 w-24 h-12 opacity-0 hover:opacity-100 transition-opacity duration-300 transition-transform hover:scale-110"
                />
              </div>
            ))}
          </div>
